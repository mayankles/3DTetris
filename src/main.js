...
const wrapAngle = a => mod(a + Math.PI, Math.PI * 2) - Math.PI;
...
let pullback = 0; // how far behind center the camera sits (see below)

function updateProjection() {
    const aspect = window.innerWidth / window.innerHeight;
    const hFovTarget = 80 * (Math.PI / 180);
    const vFov = clamp(
        2 * Math.atan(Math.tan(hFovTarget / 2) / aspect),
        55 * (Math.PI / 180),
        105 * (Math.PI / 180)
    );
    ...
    const chordHalf = INNER_R * Math.sin(THETA / 2);
...
function updateCamera(dt, now) {
    if (gyro.active) {
        ...
        if (piece) {
            const desired = gyro.yaw - pieceYaw(); // offset that would center the piece
            const err = wrapAngle(desired - gyro.yawOffsetTarget);
            const rate = now < recenterBoostUntil ? 6 : DRIFT_RATE;
...
const gyro = {
    active: false, listening: false, calibrated: false,
    yaw: 0, pitch: 0,
    yawOffset: 0, yawOffsetTarget: 0,
    pitchOffset: 0, pitchOffsetTarget: 0,
};
const DRIFT_RATE = 0.25; // rad/s of always-on assist toward the piece
let recenterBoostUntil = 0;
const isTouchDevice = 'ontouchstart' in window || navigator.maxTouchPoints > 0;

const _euler = new THREE.Euler();
const _q = new THREE.Quaternion();
const _qScreen = new THREE.Quaternion();
const _qFlip = new THREE.Quaternion(-Math.SQRT1_2, 0, 0, Math.SQRT1_2); // camera looks out the back of the phone
...
function onDeviceOrientation(e) {
    ...
    const screenAngle = ((window.screen.orientation && window.screen.orientation.angle) || window.orientation || 0) * DEG;
    _euler.set(e.beta * DEG, e.alpha * DEG, -e.gamma * DEG, 'YXZ');
    _q.setFromEuler(_euler)
        .multiply(_qFlip)
        .multiply(_qScreen.setFromAxisAngle(_zAxis, -screenAngle));
...
function requestGyro() {
    ...
    if (typeof D.requestPermission === 'function') {
        D.requestPermission().then(s => { if (s === 'granted') startGyro(); }).catch(() => {});
    } else {
        startGyro();
    }
}
...
function updateLookHint() {
    if (gyro.active) lookHint.textContent = 'MOVE PHONE TO LOOK · DOUBLE-TAP TO FACE PIECE';
    else if (pointerLocked) lookHint.textContent = 'ESC TO RELEASE MOUSE · F TO FACE PIECE';
    else if (isTouchDevice) lookHint.textContent = 'DRAG TO LOOK · DOUBLE-TAP TO FACE PIECE';
    else lookHint.textContent = 'CLICK TO ENGAGE MOUSE-LOOK · F TO FACE PIECE';
    // show on every mode change, then fade so it doesn't clutter gameplay
    lookHint.style.opacity = 1;
    clearTimeout(hintTimer);
    hintTimer = setTimeout(() => { lookHint.style.opacity = 0; }, 6000);
}
updateLookHint();
...
function gameOver() {
    ...
    piece = null;
    hideGhost();
    document.exitPointerLock?.(); // give the cursor back for the overlay
...
function bindPad(id, fn, repeat = false) {
    const el = document.getElementById(id);
    let delay = null, iv = null;
    const stop = () => {
        clearTimeout(delay); clearInterval(iv);
        delay = iv = null;
    };
    el.addEventListener('pointerdown', e => {
        ...
        if (repeat) {
            delay = setTimeout(() => {
                iv = setInterval(() => { if (state === 'playing' && piece) fn(); }, 100);
            }, 300);
        }
    });
    ['pointerup', 'pointerleave', 'pointercancel'].forEach(t => el.addEventListener(t, stop));
}
bindPad('pad-left', () => tryMove(-1, 0, 0), true);
bindPad('pad-right', () => tryMove(1, 0, 0), true);
